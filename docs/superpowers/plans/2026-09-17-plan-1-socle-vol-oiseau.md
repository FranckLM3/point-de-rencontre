...
export function zones(g: Grille, valeurs: Float32Array, listeSeuils: number[]): Tranche[] {
  ...
  return generateur(opposees)
    .map((c, rang) => ({
      seuil: listeSeuils[rang]!,
      couleur: COULEURS_TRANCHES[Math.min(rang, COULEURS_TRANCHES.length - 1)]!,
...
d3 rend les contours dans l'ordre des seuils passés (ici `-100, -200, ...`,
donc du plus étroit au plus large) ; `reverse()` met la tranche la plus
large en premier pour que les plus étroites soient dessinées par-dessus.
