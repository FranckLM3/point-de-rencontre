...
- `lignes/<i>.bin` : pour chaque gare `j` dans l'ordre de `stations.json`,
  7 octets : `minutes` (uint16, 65535 = injoignable), `km` (uint16,
  ...
  bits 5 à 7 à 0), `precedente` (uint16 : indice de la gare précédente sur
  le trajet le plus rapide vers `j` — le `de` de la dernière connexion en
  train empruntée, ou la gare quittée par une liaison à pied/urbaine ;
  65535 pour `j` = la source elle-même et pour une gare injoignable).
  Lecture : `grandeLigne = d & 1`, `correspondances = (d >> 1) & 15`. Une
  liaison à pied ou urbaine n'est pas une correspondance ; à durée égale,
  le trajet retenu est celui qui en compte le moins. Remonter `precedente`
  depuis `j` jusqu'à la source (ou 65535) donne le chemin de gares du
  trajet. Taille = 7 x nombre de gares.
